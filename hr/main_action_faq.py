...
def get_response(sentence,received_dict):
    print("main action faq ",received_dict)
    if "response_complete" in received_dict and received_dict["response_complete"] =="no":
        response = trial_response_generator.get_response(sentence,received_dict,['trial'])
        return response
    else:
        print("Line no 5 from main action",ic.classify(sentence))
    # get_res(ic.classify(sentence))
        return get_res(sentence,received_dict,ic.classify(sentence))

def get_res(sentence,received_dict,master_intent):
    ...
    if len(master_intent)==0 and sentence.lower() in ['faq']:
        ...
        if master_intent[0] == "trial":
            response = trial_response_generator.get_response(sentence,received_dict, master_intent)
